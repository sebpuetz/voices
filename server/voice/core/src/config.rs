...
use crate::{ChannelEndNotify, Ports, VoiceServerImpl};
use anyhow::Context;
...
impl VoiceServerConfig {
    ...
    pub async fn server_with_end_notify(
        &self,
        notify: impl ChannelEndNotify,
    ) -> anyhow::Result<VoiceServerImpl> {
        let ip = self.udp_host().await?;
        let ports = self.udp_ports().await?;
        Ok(VoiceServerImpl::new(ip, ports, Some(Arc::new(notify))))
    }
}
