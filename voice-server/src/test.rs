...
use crate::channel::connection::{ConnectionState, VoiceConnection};
use crate::channel::Channel;
use crate::config::VoiceServerConfig;
use crate::registry::{MockRegister, Register};
use crate::server::Server;
use crate::{EstablishSession, OpenConnection, Peer, StatusError, VoiceServerImpl};
...
async fn test_assign_chan() {
    ...
    srv.assign_channel_impl(chan_id).await.unwrap();
    let chan = srv.get_channel(chan_id).await.unwrap();
    assert!(chan.healthy());
    assert_eq!(first_port, chan.port());
    let status = srv.status_impl(chan_id).await.unwrap();
...
async fn test_chan_drop() {
    ...
    assert_matches!(
        srv.status_impl(id).await,
        Err(StatusError::ChannelNotFound(_))
    );
...
async fn test_chan_ports_exhausted() {
    ...
    for p in first_port..first_port + srv.ports.total() {
        let id = Uuid::new_v4();
        let chan = assert_matches!(srv.assign_channel_impl(id).await, Ok(chan) => chan);
        assert!(chan.healthy());
        assert_eq!(p, chan.port());
    }
    let id = Uuid::new_v4();
    assert_matches!(srv.assign_channel_impl(id).await, Err(_));
}
...
async fn test_conn_open() {
    ...
    assert_matches!(srv.status_impl(chan_id).await, Ok(status) => {
        assert!(status.is_empty());
    });
    assert_matches!(
        srv.user_status_impl(chan_id, client_id).await,
...
async fn test_conn_open_and_establish() {
    ...
    assert_matches!(srv.establish_session_impl(req).await, Ok(_));
    assert_matches!(
        srv.user_status_impl(chan_id, client_id).await,
...
async fn open(
    ...
    let conn = srv.open_connection_impl(req).await?;
...
async fn open_and_establish(
    ...
    let established = srv.establish_session_impl(req).await?;
